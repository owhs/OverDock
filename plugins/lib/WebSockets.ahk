...
class WebSockets {
    ...
    class Client extends Socket.Client {
        ...
        onRead(err) {
            ...
            if len < 126 {
                if this._bytes := this._len := len
                    (IsSet(frame) ? frame : (frame := this._frame)).Size += len, this._eptr := frame.ptr + frame.size
                if need_mask
                    goto mask
                goto payload
            }
            bytes := len == 126 ? 2 : 8
            len:
            if sz < bytes := (IsSet(bytes) ? bytes : this._bytes)
            ...
            (IsSet(frame) ? frame : (frame := this._frame)).Size += this._bytes := this._len := len, this._eptr := frame.ptr + frame.size
            if !((IsSet(need_mask) ? need_mask : ObjHasOwnProp(this, '_server')))
                goto payload
            mask:
            if sz < 4
            ...
            this._state := 'head'
            if (0x8000 & head := this._head) && len := (IsSet(len) ? len : this._len)
                this._mask_copy(p := this._eptr - len, p, len, (IsSet(v) ? v : this._mask))
            if head & 0x80
                switch head & 0xf {
                    case 0x1:	; text
                        try this.onMessage(StrGet((IsSet(frame) ? frame : this._frame), 'utf-8'))
                    case 0x2:	; bin
                        try this.onData((IsSet(frame) ? frame : this._frame))
                    case 0x8:	; close
        ...
        SendData(data, size?) => super.Send(this._create_data_frame(data, (IsSet(size) ? size : data.size)))
