...
class AudioPlugin extends OverDockPlugin {
    static Version := "1.0"
    static Description := "Displays and sets master volume."
    W := 40
    ReqWidth() {
    ...
    Render(gui, align, x, h, w) {
        ...
        this.Btn := this.AddCtrl(gui, "Text", "x" x " y0 w" Round((showPct ? 35 : this.W) * s) " h" h " 0x200 Center BackgroundTrans", Chr(0xE767))

        if (showPct) {
            gui.SetFont("s10 w500 q5 c" Config.Theme.Text, Config.Theme.Font)
            this.TxtPct := this.AddCtrl(gui, "Text", "x" (x + Round(35 * s)) " y0 w" Round(45 * s) " h" h " 0x200 Left BackgroundTrans", "100%")
    ...
    BuildSlider() {
        ...
        this.Icn := this.SldGui.Add("Text", "x" Round(15 * s) " y" Round(15 * s) " w" Round(30 * s) " h" Round(30 * s) " 0x200 BackgroundTrans", Chr(0xE767))
        this.SldGui.SetFont("s" Round(11 * s) " w500 q5 c" Config.Theme.DropText, Config.Theme.Font)
        this.Txt := this.SldGui.Add("Text", "x" Round(200 * s) " y" Round(15 * s) " w" Round(40 * s) " h" Round(30 * s) " 0x200 Right BackgroundTrans", "100%")

        try cur := SoundGetVolume()
        catch
            cur := 50
        this.CurVol := cur
        ...
        this.TrkFill := this.SldGui.Add("Text", "x" Round(45 * s) " y" Round(26 * s) " w" Round(1.5 * s * cur) " h" Round(8 * s) " Background" sliderHex " 0x0100", "")
        ...
        try {
            if HasProp(this, "Hitbox") && (hwnd == this.Hitbox.Hwnd || hwnd == this.TrkFill.Hwnd || hwnd == this.SldBg.Hwnd) {
                ...
                while GetKeyState("LButton", "P") {
                    ...
                    try SoundSetVolume(this.CurVol)
                    this.ApplyVisuals()
                    Sleep(15)
                }
            }
    ...
    ApplyVisuals() {
        ...
        this.UpdateIcon(this.CurVol)
    }
    UpdateSlider() {
        try {
            this.CurVol := SoundGetVolume()
            this.ApplyVisuals()
        }
    }
    UpdateIcon(vol) {
    ...
    Update() {
        aHwnd := 0
        if (this.App.ActivePopup)
            try aHwnd := this.App.ActivePopup.Hwnd

        if (!aHwnd || aHwnd != this.SldGui.Hwnd) {
            try {
                vol := SoundGetVolume()
                if (HasProp(this, "TxtPct") && this.TxtPct.Value != Round(vol) "%")
                    this.TxtPct.Value := Round(vol) "%"
                this.UpdateIcon(vol)
            }
        }
    }
